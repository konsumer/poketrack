...
void screen_song_update(UIState* ui) {
  ...
  if (ui->song_row > (int)song->song_len)
    ui->song_row = song->song_len;
...
void screen_song_draw(UIState* ui) {
  ...
  for (int vi = 0; vi < SONG_VISIBLE; vi++) {
    ...
    if (r == (int)song->song_len) {
      ...
      if (plus_sel)
        DrawRectangle(SONG_LABEL_W, y, CELL_W - 1, CH_H, C_CURSOR);
      ...
      if (minus_sel)
        DrawRectangle(SONG_LABEL_W + CELL_W, y, CELL_W - 1, CH_H, C_CURSOR);
    ...
    if (audio_is_playing(ui->engine)) {
      for (int ch = 0; ch < SONG_CHANNELS; ch++) {
        if (ui->engine->cursors[ch].song_row == (uint16_t)r) {
          row_bg = C_CURSOR2;
          break;
        }
      }
