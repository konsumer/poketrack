...
static const char* face_label(TrackerButton btn) {
  switch (btn) {
    case BTN_OK: return "A";
    case BTN_NO: return "B";
    case BTN_ALL: return "X";
    case BTN_NONE: return "Y";
    default: return "?";
  }
}
...
static Color face_color(TrackerButton btn) {
  switch (btn) {
    case BTN_OK: return C_A;
    case BTN_NO: return C_B;
    case BTN_ALL: return C_X;
    case BTN_NONE: return C_Y;
    default: return GRAY;
  }
}
...
static void draw_arrow(TrackerButton btn, int x, int y, int size) {
  ...
  switch (btn) {
    case BTN_UP:
      p1 = (Vector2){cx, cy - r}; p2 = (Vector2){cx - r, cy + r}; p3 = (Vector2){cx + r, cy + r};
      break;
    case BTN_DOWN:
      p1 = (Vector2){cx, cy + r}; p2 = (Vector2){cx + r, cy - r}; p3 = (Vector2){cx - r, cy - r};
      break;
    case BTN_LEFT:
      p1 = (Vector2){cx - r, cy}; p2 = (Vector2){cx + r, cy + r}; p3 = (Vector2){cx + r, cy - r};
      break;
    default:  // BTN_RIGHT
      p1 = (Vector2){cx + r, cy}; p2 = (Vector2){cx - r, cy - r}; p3 = (Vector2){cx - r, cy + r};
      break;
...
static const char* key_label(int key) {
  switch (key) {
    ...
    case KEY_Q: return "Q";
    case KEY_W: return "W";
    case KEY_RIGHT_SHIFT: return "SH";
    default: return "?";
  }
}
...
static bool token_to_btn(const char* name, int len, TrackerButton* out) {
  static const struct { const char* n; TrackerButton b; } table[] = {
      {"UP", BTN_UP}, {"DOWN", BTN_DOWN}, {"LEFT", BTN_LEFT}, {"RIGHT", BTN_RIGHT},
      {"OK", BTN_OK}, {"NO", BTN_NO}, {"ALL", BTN_ALL}, {"NONE", BTN_NONE},
      {"PREV", BTN_PREV}, {"NEXT", BTN_NEXT}, {"PLAY", BTN_PLAY}, {"SCREEN", BTN_SCREEN},
  };
